...
from newCIB import CIBForSequenceClassification
# from cib0 import CIBForSequenceClassification
...
argParser.add_argument("--dataset", type=str,
                    choices=["mosi", "mosei", "mustard_plus_plus"], default="mustard_plus_plus_u_fine")
argParser.add_argument("-seed", "--seed", default=42, type=int, help="SEED value")
...
argParser.add_argument("-b", "--batch_size", default=16, type=int, help="Batch Size")
...
argParser.add_argument('--inter_dim', default=256, help='dimension of inter layers', type=int)
...
argParser.add_argument('--B_dim', default=128, type=float)
...
def main():
    ...
    p_lambda_value = [8, 16]
    p_beta_values = [32, 16]
    p_gamma_values = [16, 32]
    p_sigma_value = [32, 16]
    B_dim_values = [128, 256]

    # 生成所有参数组合
    parameter_combinations = list(
        itertools.product(p_lambda_value, p_beta_values, p_gamma_values, p_sigma_value, B_dim_values))

    # 循环遍历所有参数组合
    for p_lambda, p_beta, p_gamma, p_sigma, B_dim in parameter_combinations:
        # 恢复默认参数
        ...
        # 打开保存结果的文件
        result_filename = f'newCIB_fine_{p_lambda}_{p_beta}_{p_gamma}_{p_sigma}_{B_dim}.txt'
